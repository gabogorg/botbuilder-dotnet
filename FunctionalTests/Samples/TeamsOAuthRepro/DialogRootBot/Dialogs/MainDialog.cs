...
#pragma warning restore SA1202 // Elements should be ordered by access
        ...
        private Activity CreateDialogSkillBotActivity(string selectedOption, ITurnContext turnContext)
        {
            ...
            if (selectedOption.StartsWith("logout", StringComparison.CurrentCultureIgnoreCase))
            {
                activity = (Activity)Activity.CreateMessageActivity();
                activity.Text = "logout";
            }

            // Send an event activity to the skill with "GetWeather" in the name and some testing values.
            if (selectedOption.StartsWith(SkillActionGetWeather, StringComparison.CurrentCultureIgnoreCase))
            {
                activity = (Activity)Activity.CreateMessageActivity();
                activity.Text = "hi";
                ...
                //activity.Value = JObject.Parse("{ \"date\": \"2020-10-22\", \"timezone\": \"PST\"}");
            }
